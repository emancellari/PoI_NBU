...
def bagnet33(pretrained=False, strides=[2, 2, 2, 1], **kwargs):
    ...
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(model_urls['bagnet33'], map_location='cpu')
...
def bagnet17(pretrained=False, strides=[2, 2, 2, 1], **kwargs):
    ...
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(model_urls['bagnet17'], map_location='cpu')
        model.load_state_dict(state_dict)
    return model
...
def bagnet9(pretrained=False, strides=[2, 2, 2, 1], **kwargs):
    ...
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(model_urls['bagnet9'], map_location='cpu')
        model.load_state_dict(state_dict)
    return model